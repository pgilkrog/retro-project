...
export class InventoryUI extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    const panel =
      isPlayer === true ? this.createUserBackground(scene) : this.createNPCBackground(scene)
    panel.setScale(isPlayer === true ? 0.5 : 1)
    this.add(panel)
  ...
  private createItemElement(item: InventoryItem, index: number) {
    ...
    const image = new Phaser.GameObjects.Image(this.scene, 0, 0, item.item.image)
    image.setScale(0.5)
    itemElement.add(image)

    if (item.quantity > 1) {
      const quantityLabel = new Phaser.GameObjects.Text(this.scene, 5, 5, `x${item.quantity}`, {
        fontSize: '16',
      })
  ...
  private createUserBackground(scene: Phaser.Scene) {
    return new Phaser.GameObjects.Image(scene, 0, 0, 'inventoryUI')
  }

  private createNPCBackground(scene: Phaser.Scene) {
    return new Phaser.GameObjects.Rectangle(scene, 0, 0, 400, 300, 0x000000, 0.5)
  }

  private createBacks(scene: Phaser.Scene, isPlayer: boolean) {
    const back = new Phaser.GameObjects.Rectangle(scene, -148, 0, 100, 300, 0x000000, 0.5)
    const back2 = new Phaser.GameObjects.Rectangle(scene, 152, 0, 100, 300, 0x000000, 0.5)
